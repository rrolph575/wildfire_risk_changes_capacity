...
| [summary_of_validation_against_observed_fires.md](summary_of_validation_against_observed_fires.md) | **Validation.** Checked the products against 1,050 MTBS wildfires (all ownerships). Fire weather is **anti-predictive** of where fires burn (lift 0.25x TVA / 0.42x SoCal); `fuel_index` alone is the best predictor (1.87x / 1.83x). Does NOT invalidate the climate-change signal, which is within-cell; DOES mean the absolute classes read as fire-weather severity, not fire likelihood. Method kept unchanged deliberately. | `plot_fire_validation.py` | `data/fire_perimeters/` |
